    ...
    def call
      {
        ...
        product_code: extract_by_regex(/c[oó]digo\s*([A-Z0-9]+)/i) || extract_by_regex(/produto\s+abc123/i) || extract_by_regex(/Produto de interesse:\s*(.+)/i) || extract_by_regex(/c[oó]digo do produto:\s*(.+)/i),
